...
# Authors (in alphabetical order): Amadeo, Bautista, Lingad
    ...
    for test_filename, test_image in test_images:
        ...
        for match in matches:
            if len(match) == 2 and match[1].distance != 0:
                m, n = match
                if m.distance/n.distance <= 0.75: # 25% closer points are considered as good matches | the smaller the number, the bigger scope of good matches, but less strict
        ...
        if len(good_matches) > 10:
            ...
            # Pre-defined threshold for min. number of inliers to accept as a match: 10
