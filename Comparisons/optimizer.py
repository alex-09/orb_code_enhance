...
def optimize_nfeatures(query_image, query_filename, test_images, estimator, preprocess_img):
    def objective(nfeatures):
        matches_info = find_matches(query_image, query_filename, test_images, nfeatures, estimator, preprocess_img)
        if matches_info and len(matches_info[0]) > 0:
            ...
            #                               len(query_features), len(test_features), first_good_matches)
            _, _, inliers_count, _, total_matches, _, _, first_good_matches = matches_info[0][0]
